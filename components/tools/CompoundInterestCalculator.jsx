...
export default function CompoundInterestCalculator() {
  ...
  const copyToClipboard = async () => {
    if (!result) return;
    await navigator.clipboard.writeText(`Future Value: ${result.amount}, Compound Interest: ${result.interest}`);
    setCopied(true);
              ...
              {result ? (
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    Compound Interest Earned: <span className="font-semibold text-gray-700">{result.interest}</span>
                  </p>
                </div>
              ) : (
