...
export default function SimpleInterestCalculator() {
  ...
  const calculate = () => {
    ...
    if (!principal || !rate || !years) { setResult(null); return; }
  ...
  const copyToClipboard = async () => {
    if (!result) return;
    await navigator.clipboard.writeText(`Simple Interest: ${result.interest}, Total Amount: ${result.total}`);
    setCopied(true);
        ...
        <div className="max-w-3xl mx-auto border border-gray-200 rounded-2xl p-5 md:p-6 shadow-sm">
          ...
          <div className="mb-5">
            ...
            <div className="rounded-xl border border-gray-300 bg-gray-50 p-5">
              {result ? (
                <div>
                  <p className="text-sm text-gray-500 mb-1">Simple Interest</p>
                  <p className="text-3xl font-bold text-blue-600">
                    {result.interest}
                  </p>
                  <p className="text-sm text-gray-500 mt-2">
                    Total Amount: <span className="font-semibold text-gray-700">{result.total}</span>
                  </p>
                </div>
              ) : (
