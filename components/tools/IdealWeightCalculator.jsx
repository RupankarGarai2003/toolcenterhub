...
export default function IdealWeightCalculator() {
  ...
  const calculate = () => {
    ...
    if (!height) { setResult(null); return; }
    const h = parseFloat(height);
    const inches = h / 2.54;
    const overFive = Math.max(0, inches - 60);
    let ideal;
    if (gender === "male") {
      ideal = 50 + 2.3 * overFive;
    } else {
      ideal = 45.5 + 2.3 * overFive;
    }
    setResult({ ideal: ideal.toFixed(1) });
  
  };

  const copyToClipboard = async () => {
    if (!result) return;
    await navigator.clipboard.writeText(`Ideal Weight: ${result.ideal} kg`);
    setCopied(true);
        ...
        <div className="max-w-3xl mx-auto border border-gray-200 rounded-2xl p-5 md:p-6 shadow-sm">
          ...
          <div className="mb-5">
            ...
            <div className="rounded-xl border border-gray-300 bg-gray-50 p-5">
              {result ? (
                <div>
                  <p className="text-sm text-gray-500 mb-1">Ideal Weight</p>
